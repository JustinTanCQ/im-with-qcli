...
RECOGNITION_MODEL_ID = os.environ.get("RECOGNITION_MODEL_ID", "anthropic.claude-3-5-haiku-20241022-v1:0")
BEDROCK_REGION = os.environ.get("BEDROCK_REGION", "us-west-2")
KNOWLEDGE_SUMMARY = os.environ.get("KNOWLEDGE_SUMMARY", "")
...
def check_if_valid_question(text: str) -> bool:
    ...
    if KNOWLEDGE_SUMMARY == "":
        prompt = f"""
        请判断以下<问题></问题>中的内容是否是关于 "AWS和软件开发相关"
        如果是，回答 "AWS"；若不是，回答 "否"
        ...
        prompt = f"""
        请判断以下 <问题></问题> 中的内容是否是关于 "AWS和软件开发相关" 或严格匹配<企业知识库></企业知识库>中的内容范围
        <企业知识库>{KNOWLEDGE_SUMMARY}</企业知识库>

        <问题>{text}</问题>

        若是关于 "AWS和软件开发相关"，回答 "AWS"；若是关于 "企业知识库的内容范围"，回答 "知识库"；若都不是，回答 "否"
        """
